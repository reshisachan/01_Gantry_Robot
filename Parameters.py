...
#########------------------------Set Operating mode of motor to Position / Velocity /Homig Mode--------------------##########
...
def Start_Motor_Run_Position_mode(SlaveID):
    write  = client.write_register(0x6040,0x001F,SlaveID) # Start running motor
    write  = client.write_register(0x6040,0x000F,SlaveID) # The next fast sampling
...
#########--------------------------Function to Set Hopper_Motor_Position_and Run it ------------------------------------------##########
...
#########--------------------------Function to Set Hopper_Motor_speed_and Run it ---------------------------------------------##########

def Run_Motor_in_speed_mode():
    Set_Target_speed(Hopper_Z_axis_slaveid,Hopper_Z_Axis_speed)
    Set_Motor_Acceleration(Hopper_Z_axis_slaveid,Hopper_Z_Axis_acceleration)
    Set_Motor_Deacceleration(Hopper_Z_axis_slaveid,Hopper_Z_Axis_deacceleration)
    Start_Motor_Run_Speed_mode(Hopper_Z_axis_slaveid)
